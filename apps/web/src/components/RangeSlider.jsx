...
// Dual-thumb range slider on a single bar (drag the left thumb to raise the floor,
// the right thumb to lower the ceiling) plus typed min/max number inputs. Both stay
// in sync. `format` is used only for the value labels; the number inputs are raw.
export default function RangeSlider({ min, max, step = 1, low, high, onChange, prefix = '', suffix = '' }) {
  ...
  const numClass = 'h-9 w-full rounded-lg border border-ink-200 text-sm text-ink-800 outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-200'
    ...
    <div>
      <div className="relative h-5">
        ...
        <input
          ...
          // keep the min thumb grabbable when it's pushed up near the max
          style={{ zIndex: low > max - range * 0.08 ? 5 : 3 }}
          aria-label="Minimum"
        />
        <input
      ...
      <div className="mt-3 flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            type="number" inputMode="numeric" min={min} max={high} step={step}
            value={loText}
            onChange={(e) => setLoText(e.target.value)}
            onBlur={(e) => setLow(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter') setLow(e.currentTarget.value) }}
            className={`${numClass} ${prefix ? 'pl-9' : 'pl-2.5'} ${suffix ? 'pr-9' : 'pr-2.5'}`}
        ...
        <div className="relative flex-1">
          {prefix && <span className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-ink-400">{prefix}</span>}
          <input
            type="number" inputMode="numeric" min={low} max={max} step={step}
            value={hiText}
            onChange={(e) => setHiText(e.target.value)}
            onBlur={(e) => setHigh(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter') setHigh(e.currentTarget.value) }}
            className={`${numClass} ${prefix ? 'pl-9' : 'pl-2.5'} ${suffix ? 'pr-9' : 'pr-2.5'}`}
